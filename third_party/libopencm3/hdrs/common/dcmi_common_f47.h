 ...
 *
 * DCMI_RIS gives the raw interrupt status and is accessible in read only. When
 * read, this register returns the status of the corresponding interrupt before
 * masking with the DCMI_IER register value.
 */
 ...
 *
 * The DCMI_IER register is used to enable interrupts. When one of the DCMI_IER
 * bits is set, the corresponding interrupt is enabled. This register is
 * accessible in both read and write.
 */
 ...
 *
 * This DCMI_MIS register is a read-only register. When read, it returns the
 * current masked status value (depending on the value in DCMI_IER) of the
 * corresponding interrupt. A bit in this register is set if the corresponding
 * enable bit in DCMI_IER is set and the corresponding bit in DCMI_RIS is set.
 */
 ...
 *
 * The DCMI_ICR register is write-only. Writing a ‘1’ into a bit of this
 * register clears the corresponding bit in the DCMI_RIS and DCMI_MIS registers.
 ...
 * DCMI data register
 *
 * The digital camera Interface packages all the received data in 32-bit format
 * before requesting a DMA transfer. A 4-word deep FIFO is available to leave
 * enough time for DMA transfers and avoid DMA overrun conditions.
 */
