 ...
 * @ingroup CM3_mpu_defines
 * The MPU_TYPE register is always available, even if the MPU is not
 *implemented. In that case, the DREGION field will read as 0.
 *@{*/
#define MPU_TYPE_IREGION_LSB 16
#define MPU_TYPE_IREGION                                                \
    (0xFF << MPU_TYPE_IREGION_LSB) /**< Number of protected instruction \
                                      regions; always 0 on v6m/v7m */
#define MPU_TYPE_DREGION_LSB 8
#define MPU_TYPE_DREGION \
    (0xFF << MPU_TYPE_DREGION_LSB) /**< Number of protected data regions */
#define MPU_TYPE_SEPARATE                                                 \
    (1 << 0) /**< Indicates if instruction regions are separate from data \
                regions; always 0 on v6m/v7m */
/**@}*/
...
#define MPU_CTRL_PRIVDEFENA \
    (1 << 2) /**< Enable default map in privileged mode */
#define MPU_CTRL_HFNMIENA \
    (1 << 1) /**< Enable MPU during hard fault, NMI, and FAULTMASK handlers */
...
#define MPU_RNR_REGION                                                       \
    (0xFF << MPU_RNR_REGION_LSB) /**< Determines the region affected by RBAR \
                                    and RASR */
/**@}*/
...
#define MPU_RBAR_VALID                                                     \
    (1 << 4) /**< Use REGION to determine region to be accessed instead of \
                MPU_RNR */
#define MPU_RBAR_REGION_LSB 0
#define MPU_RBAR_REGION \
    (0xF                \
     << MPU_RBAR_REGION_LSB) /**< Region to change if MPU_RBAR_VALID is set */
/**@}*/
...
#define MPU_RASR_ATTR_TEX \
    (7 << 19) /**< Type extension (e.g., memory ordering) */
