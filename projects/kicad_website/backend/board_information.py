...
def Home():
    if request.method == 'POST':
        ...
        updated_board_files = np.reshape(sorted(updated_board_files), [int(len(updated_board_files)/2), 2])

        for curr_ibom, curr_schematic in updated_board_files:
            curr_board_name = '.'.join((curr_ibom.split('/')[3:-1]))
            curr_schematic_link = "https://oem-outline.nyc3.digitaloceanspaces.com/kicad-artifacts/" + curr_schematic
            curr_ibom_link = "https://oem-outline.nyc3.digitaloceanspaces.com/kicad-artifacts/" + curr_ibom

            with sqlite3.connect("database.db") as con:
                cur = con.cursor()
                cur.execute("INSERT INTO kicad_artifacts (board_name,commits,schematic,ibom) VALUES (?,?,?,?)", (curr_board_name, commit_hash, curr_schematic_link, curr_ibom_link))
                con.commit()
    ...
    if request.method == 'GET':
        ...
        for key, value in artifacts.items():
            ...
            artifacts[key][0] = {most_recent_commit + ' (Current Version)': value[0][most_recent_commit]}
            print(artifacts[key])
...
def board_name(board_name):
    ...
    for row in rows:
        if row[0] == board_name:
            ...
            board_counter += 1
    if files_by_commit != {}:
        return render_template('board.html', board_name=board_name, files_by_commit=files_by_commit)
...
def board_name_commit(board_name,commit):
    ...
    for row in rows:
        if row[0] == board_name and row[1] == commit:
            schematic = row[2]
            bom = row[3]
            return render_template('board_commit.html', board_name=board_name, commit=commit, schematic = schematic, bom = bom)
    return render_template('error.html')
