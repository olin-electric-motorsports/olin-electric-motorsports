...
class Pin:
    """Abstract defition of a "pin" on the HitL that can be connected to a
    target device.

    The ``Pin`` class allows us to define a generic pin that can either be input
    or output, and either digital or analog. This allows us to have more
    ...
    def set(self, val: Union[int, float]):
        if self.pinmode != PinMode.OUTPUT:
...
class IOController:
    def __init__(self, pins):
        ...
        # This allows users to access the pins by doing something like
        #   io.brake_pressure.set(0.23)
