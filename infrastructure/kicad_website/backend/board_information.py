...
def TableOfContents():
    if request.method == 'POST':
        ...
        while current_board_counter < len(buildable_list):
            board_file_split = re.split('//|/|:', buildable_list[current_board_counter])
            current_board_name = f'{board_file_split[-2]}'
            i = -3
            while board_file_split[i] != "hardware":
                current_board_name = f'{board_file_split[i]}/{current_board_name}'
                i -= 1
