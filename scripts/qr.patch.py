...
from kicad_qrcode import QRCode, ErrorCorrectLevel  # TODO: local qrcode package is preferred, so we renamed it
...
class QRCodeWizard(FootprintWizardBase.FootprintWizard):
    ...
    def CheckParameters(self):
        ...
        self.errorCorrectionLevel = str(self.parameters['Barcode']['Error Correction Level (L|M|Q|H)'])

        if self.border < 0:
            self.border = 0

        assert self.errorCorrectionLevel in ErrorCorrectionLevels.keys(), "Invalid Error Correction Level"
        ...
        #self.qr.make()
        self.qr = QRCode.getMinimumQRCode(str(self.Barcode), ErrorCorrectionLevels[self.errorCorrectionLevel])
