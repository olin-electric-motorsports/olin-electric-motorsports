...
// To Program:
// SET_FUSES=1 bazel run --config=16m1 //vehicle/mkvi/software/charging -c opt
// -- -c usbasp
...
//         chgMsg.charging_voltage = (float)((uint16_t)rx_msg[0] << 8 |
//         rx_msg[1]) * 0.1; chgMsg.charging_current =
//         (float)((uint16_t)rx_msg[2] << 8 | rx_msg[3]) * 0.1;
...
int main(void) {
    ...
    while (1) {
        ...
        if (send_can) {
            ...
            // Broker exchange with the car (get charging targets and OK from
            // BMS core)
