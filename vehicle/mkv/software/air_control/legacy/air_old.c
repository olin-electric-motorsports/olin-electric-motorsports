...
#define FAULT_CODE_DISCHARGE_STUCK \
    0X09 // unused because of difficulty of detection
#define FAULT_CODE_DISCHARGE_CONTROL_LOSS 0X0A
#define FAULT_CODE_PRECHARGE_INCOMPLETE \
    0X0B // if precharge is too slow or stopping below
         // MINIMUM_VOLTAGE_AFTER_PRECHARGE
...
#define MOB_IBH \
    5 // TODO change this to panic message? probably should listen for panics
...
#define OVF_COUNT_PRECHARGING \
    0x10 // roughly 2 seconds -> 3 time constants (1/(2*pi*1k*440uF))*3
         // = 1.08ish
#define OVF_COUNT_DISCHARGING \
    0x1F // roughly 4 seconds -> 3 times as long as precharge (3k res vs 1k res)
         // -> 3.24ish

volatile uint8_t gFlag = 0x00; // Global Flag. Tells us that gFlag will change
                               // and to assign it to an unsigned 8bit integer
...
ISR(PCINT1_vect) { // PCINT14, 15, 8, 9 -> AIRPLUS_AUX, AIRMINUS_AUX, SS_IMD,
                   // SS_BMS
...
ISR(CAN_INT_vect) {
    ...
    if (bit_is_set(CANSTMOB, RXOK)) {
        ...
        CANSTMOB = 0x00;
        CAN_wait_on_receive(MOB_BMS, CAN_ID_BMS_CORE, CAN_LEN_BMS_CORE,
                            CAN_MSK_SINGLE);
    }
    ...
    void initTimer1(void) {
        ...
        // 4MHz CPU, prescaler 8, 16-bit timer overflow -> (4000000/8)/(2^16-1)
        // =  7.63 Hz
    ...
    void resetTimer1(void) {
        cli();
        TCNT1H = 0x00; // write timer count to 0, high byte must be written
                       // first per datasheet
        TCNT1L = 0x00;
        sei();
    ...
    void checkBMSPowerStagePlausibility(
        void) { // tested and working. needs some type of delay as to when the
                // check is made so it's not too soon
        if ((bit_is_set(gFlag, FLAG_BMS_STATUS)
             && bit_is_set(sFlag, FLAG_SS_TSCONN)
             && bit_is_clear(sFlag, FLAG_SS_BMS))
            || (bit_is_clear(gFlag, FLAG_BMS_STATUS)
                && bit_is_set(sFlag, FLAG_SS_BMS))) {
    ...
    void checkIMDPowerStagePlausibility(void) { // tested and working
        if ((bit_is_set(gFlag, FLAG_IMD_STATUS)
             && bit_is_set(sFlag, FLAG_SS_BMS)
             && bit_is_clear(sFlag, FLAG_SS_IMD))
            || (bit_is_clear(gFlag, FLAG_IMD_STATUS)
                && bit_is_set(sFlag, FLAG_SS_IMD))) {
            msgCritical[MSG_INDEX_ERROR_CODE] = FAULT_CODE_IMD_IMPLAUSIBILITY;
        }
    }

    void checkAIRPLUS(void) {
        if (bit_is_set(gFlag, FLAG_AIRPLUS_WELD_DET)
            && bit_is_clear(gFlag, FLAG_TSMS_STATUS)) {
            msgCritical[MSG_INDEX_ERROR_CODE] = FAULT_CODE_AIRPLUS_WELD;
        } else if (bit_is_clear(gFlag, FLAG_AIRPLUS_WELD_DET)
                   && bit_is_set(gFlag, FLAG_TSMS_STATUS)) {
            msgCritical[MSG_INDEX_ERROR_CODE] = FAULT_CODE_AIRPLUS_CONTROL_LOSS;
        }
    }

    void checkAIRMINUS(void) {
        if (bit_is_clear(AIRMINUS_PORT, AIRMINUS_LSD)
            && bit_is_set(gFlag, FLAG_AIRMINUS_WELD_DET)) {
            msgCritical[MSG_INDEX_ERROR_CODE] = FAULT_CODE_AIRMINUS_WELD;
        } else if (bit_is_set(AIRMINUS_PORT, AIRMINUS_LSD)
                   && bit_is_clear(gFlag, FLAG_AIRMINUS_WELD_DET)) {
            msgCritical[MSG_INDEX_ERROR_CODE]
                = FAULT_CODE_AIRMINUS_CONTROL_LOSS;
        }
    }

    void conditionalMessageSet(uint8_t reg, uint8_t bit, uint8_t msg[],
                               uint8_t index, uint8_t condHigh,
                               uint8_t condLow) {
    ...
    void sendShutdownSenseCANMessage(void) {
        msgShutdownSense[MSG_INDEX_SS_TSCONN]
            = bit_is_set(sFlag, FLAG_SS_TSCONN) ? 0xff : 0x00;
        msgShutdownSense[MSG_INDEX_SS_HVD]
            = bit_is_set(sFlag, FLAG_SS_HVD) ? 0xff : 0x00;
        msgShutdownSense[MSG_INDEX_SS_IMD]
            = bit_is_set(sFlag, FLAG_SS_IMD) ? 0xff : 0x00;
        msgShutdownSense[MSG_INDEX_SS_BMS_STATUS]
            = bit_is_set(sFlag, FLAG_SS_BMS_STATUS) ? 0xff : 0x00;
        msgShutdownSense[MSG_INDEX_SS_IMD_STATUS]
            = bit_is_set(sFlag, FLAG_SS_IMD_STATUS) ? 0xff : 0x00;
        msgShutdownSense[MSG_INDEX_SS_TSMS]
            = bit_is_set(sFlag, FLAG_SS_TSMS) ? 0xff : 0x00;
        CAN_transmit(MOB_BROADCAST_SS, CAN_ID_AIR_CONTROL_SENSE,
                     CAN_LEN_AIR_CONTROL_SENSE, msgShutdownSense);
    }

    void sendCriticalCANMessage(void) {
        // TODO -> hv check?
        conditionalMessageSet(gFlag, FLAG_AIRPLUS_WELD_DET, msgCritical,
                              MSG_INDEX_AIRPLUS_AUX, 0xff, 0x00);
        conditionalMessageSet(gFlag, FLAG_AIRMINUS_WELD_DET, msgCritical,
                              MSG_INDEX_AIRMINUS_AUX, 0xff, 0x00);
        CAN_transmit(MOB_BROADCAST_CRITICAL, CAN_ID_AIR_CONTROL_CRITICAL,
                     CAN_LEN_AIR_CONTROL_CRITICAL, msgCritical);
        ...
        int main(void) {
            ...
            CAN_wait_on_receive(MOB_BRAKELIGHT, CAN_ID_BRAKE_LIGHT,
                                CAN_LEN_BRAKE_LIGHT, CAN_MSK_SINGLE);

            // Enable interrupt
            PCICR |= _BV(PCIE0) | _BV(PCIE1) | _BV(PCIE2);
            PCMSK0 |= _BV(PCINT3) | _BV(PCINT4) | _BV(PCINT5) | _BV(PCINT6)
                      | _BV(PCINT7);
            ...
            readAllInputs(); // in case they are set high before micro starts up
                             // and therefore won't trigger an interrupt
            ...
            while (charging) {
                if (bit_is_set(gFlag, UPDATE_STATUS)) {
                    ...
                    gFlag &= ~_BV(UPDATE_STATUS); // TODO IMD STATUS PIN TURNS
                                                  // OFF SLOW DEBUG
            ...
            while (1) {
                if (bit_is_set(gFlag, UPDATE_STATUS)) {
                    ...
                    checkIMDPowerStagePlausibility(); // delay this for IMD
                                                      // startup time
                    // checkAIRPLUS(); // TODO think about charging, currently
                    // compares AIR plus to TSMS can message, won't work in
                    // charging checkAIRMINUS();
                    sendShutdownSenseCANMessage();
                    sendCriticalCANMessage();

                    if (tractiveSystemStatus == TS_STATUS_DEENERGIZED) {
                        PRECHARGE_PORT
                            &= ~_BV(PRECHARGE_CTRL); // open precharge relay,
                                                     // sanity check
                        RJ45_LED_PORT &= ~_BV(RJ45_LED1);
                        AIRMINUS_PORT &= ~_BV(
                            AIRMINUS_LSD); // open air minus, sanity check
                        RJ45_LED_PORT &= ~_BV(RJ45_LED2);
                        if (bit_is_set(
                                gFlag,
                                FLAG_AIRPLUS_WELD_DET)) { // if tsms closed
                            char tsms_closed[] = "tsms_closed";
                            LOG_println(tsms_closed, strlen(tsms_closed));
                            char precharge_delay[] = "precharge_delay";
                            LOG_println(precharge_delay,
                                        strlen(precharge_delay));
                            tractiveSystemStatus
                                = TS_STATUS_PRECHARGE_DELAY; // set status to
                                                             // precharge delay
                            resetTimer1(); // reset timer 1
                        }
                    } else if (tractiveSystemStatus
                               == TS_STATUS_PRECHARGE_DELAY) {
                        if (bit_is_clear(
                                gFlag,
                                FLAG_AIRPLUS_WELD_DET)) { // IS THIS RIGHT???
                                                          // last year was
                                                          // FLAG_TSMS_STATUS
                        ...
                        } else if (timer1OverflowCount
                                   > OVF_COUNT_PRECHARGE_DELAY) { // if
                                                                  // precharge
                                                                  // delay time
                                                                  // elapsed
                            char precharge_delay_over[]
                                = "precharge_delay_over";
                            LOG_println(precharge_delay_over,
                                        strlen(precharge_delay_over));
                            tractiveSystemStatus
                                = TS_STATUS_PRECHARGING; // set status to
                                                         // precharging
                            msgCritical[MSG_INDEX_PRECHARGE_STATUS]
                                = 0x0f; // update critical can message to
                                        // precharge started
                            PRECHARGE_PORT
                                |= _BV(PRECHARGE_CTRL); // close precharge relay
                            RJ45_LED_PORT |= _BV(RJ45_LED1);
                            resetTimer1(); // reset timer 1
                        ...
                        if (bit_is_clear(gFlag, FLAG_TSMS_STATUS)) {
                            ...
                            RJ45_LED_PORT &= ~_BV(RJ45_LED1);
                            msgCritical[MSG_INDEX_PRECHARGE_STATUS]
                                = 0x00; // update critical can message to
                                        // precharge not started
                            resetTimer1();
                        } else if (timer1OverflowCount
                                   > OVF_COUNT_PRECHARGING) {
                            char precharge_over[] = "precharge_over";
                            LOG_println(
                                precharge_over,
                                strlen(precharge_over)); // if precharging time
                                                         // elapsed
                            AIRMINUS_PORT
                                |= _BV(AIRMINUS_LSD); // close air minus
                            RJ45_LED_PORT |= _BV(RJ45_LED2);
                            _delay_ms(
                                100); // let air minus close before we do stuff
                            // checkAIRMINUS(); // confirm closure
                            PRECHARGE_PORT
                                &= ~_BV(PRECHARGE_CTRL); // open precharge relay
                            RJ45_LED_PORT &= ~_BV(RJ45_LED1);
                            tractiveSystemStatus
                                = TS_STATUS_ENERGIZED; // set status to
                                                       // energized
                            msgCritical[MSG_INDEX_PRECHARGE_STATUS]
                                = 0xff; // update critical can message to
                                        // precharge complete
                        ...
                        if (bit_is_clear(
                            ...
                            LOG_println(discharging, strlen(discharging));
                            AIRMINUS_PORT
                                &= ~_BV(AIRMINUS_LSD); // open air minus
                            RJ45_LED_PORT &= ~_BV(RJ45_LED2);
                            msgCritical[MSG_INDEX_PRECHARGE_STATUS]
                                = 0x00; // update critical can message to
                                        // precharge not started
                            tractiveSystemStatus
                                = TS_STATUS_DISCHARGING; // set status to
                                                         // discharging
                            resetTimer1(); // reset timer 1
                        }
                    } else if (tractiveSystemStatus == TS_STATUS_DISCHARGING) {
                        if (timer1OverflowCount
                            > OVF_COUNT_DISCHARGING) { // if discharging time
                                                       // elapsed
                            char discharge_over[] = "discharge_over";
                            LOG_println(discharge_over, strlen(discharge_over));
                            tractiveSystemStatus
                                = TS_STATUS_DEENERGIZED; // set status to
                                                         // deenergized
                            char deenergized[] = "deenergized";
                            LOG_println(deenergized, strlen(deenergized));
                        }
                        ...
                        AIRMINUS_PORT &= ~_BV(
                            AIRMINUS_LSD); // open air minus and precharge
                        RJ45_LED_PORT &= ~_BV(RJ45_LED2);
                        PRECHARGE_PORT &= ~_BV(PRECHARGE_CTRL);
                        RJ45_LED_PORT &= ~_BV(RJ45_LED1);
                        msgCritical[MSG_INDEX_ERROR_CODE]
                            = FAULT_CODE_GENERAL; // see that panic keeps being
                                                  // sent...
                    }
                }
            }
