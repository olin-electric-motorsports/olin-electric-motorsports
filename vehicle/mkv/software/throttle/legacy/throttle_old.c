...
void initMC(void) {
    ...
    CAN_transmit(MOB_MOTORCONTROLLER, CAN_ID_MC_COMMAND, CAN_LEN_MC_COMMAND,
                 gCANMotorController);
}
...
void sendCanMessages(int viewCan) {
    ...
    CAN_transmit(MOB_BROADCAST, CAN_ID_THROTTLE, CAN_LEN_THROTTLE, gCANMessage);
...
int main(void) {
    ...
    while (1) {
        if (bit_is_set(gTimerFlag,
                       UPDATE_STATUS)) { // everytime timer goes off, clear
                                         // flag, store throttle, etc.
            gTimerFlag &= ~_BV(UPDATE_STATUS);
