...
#define VERBOSE \
    1 // this will print PEC error count each cycle, any temperature and voltage
      // cells that fault,
//                    the voltages of a whole segment if it has an overvoltage
//                    fault
...
const uint16_t OV_THRESHOLD = 42000; // 35900; // Over voltage threshold ADC
                                     // Code
const uint16_t UV_THRESHOLD = 30000; // 20000;// Under voltage threshold ADC
                                     // Code
...
int8_t read_all_voltages(void) // Start Cell ADC Measurement
{
    ...
    if (error == 0) {
        ...
        for (uint8_t ic = 0; ic < TOTAL_IC; ic++) {
            for (uint8_t cell = 0; cell < NUM_CELLS; cell++) {
                ...
                if ((cell == 4) || (cell == 5) || (cell == 10) || (cell == 11))
                    continue;

                uint16_t cell_value = cell_voltages[ic][cell];

                if (cell_value > OV_THRESHOLD) {
                    gFlag |= OVER_VOLTAGE; // left out for now to prevent
                                           // shutdown on overvolted cell
#if (VERBOSE == 1)
                    sprintf(temp_msg, "OV ic: %u cell: %u v: %u", ic, cell,
                            cell_value);
                    ...
                    gFlag |= UNDER_VOLTAGE;
#if (VERBOSE == 1)
                    sprintf(temp_msg, "UV ic: %u cell: %u v: %u", ic, cell,
                            cell_value);
                    LOG_println(temp_msg, strlen(temp_msg));
#endif
                }
    ...
    for (int i = 0; i < TOTAL_IC; i++) {
        sprintf(temp_msg,
                ...
                "%u,%u,"
                "%u,%u",
                i, error, cell_voltages[i][0], cell_voltages[i][1],
                cell_voltages[i][2], cell_voltages[i][3], cell_voltages[i][6],
                cell_voltages[i][7], cell_voltages[i][8], cell_voltages[i][9]);
...
int8_t read_all_temperatures(void) {
    ...
    for (uint8_t i = 0; i < 4; i++) {
        ...
        ltc6811_adax(MD_7KHZ_3KHZ,
                     AUX_CH_GPIO1); // start ADC measurement for GPIO CH 1
        ltc6811_pollAdc();
        error = ltc6811_rdaux(0, TOTAL_IC,
                              _aux_voltages); // Parse all ADC measurements back

        // Grab aux voltages into the temp voltages array
        for (uint8_t ic = 0; ic < TOTAL_IC; ic++) {
            // First four are out of order, 0123 are 2_2, 2_1, 1_2, and 1_1
            // respectively
            temp_sensor_voltages[ic][3 - i]
                = _aux_voltages[ic][0]; // Store temperatures
        }
    }
    ...
    for (uint8_t i = 0; i < MUX_CHANNELS; i++) {
        ...
        for (uint8_t ic = 0; ic < TOTAL_IC; ic++) {
            ...
            temp_sensor_voltages[ic][NUM_TEMPS - 9 - i]
                = _aux_voltages[ic][0]; // Store temperatures
        }
    }
    ...
    for (uint8_t i = 0; i < MUX_CHANNELS; i++) {
        ...
        // Grab aux voltages into the temp voltages array
        for (uint8_t ic = 0; ic < TOTAL_IC; ic++) {
            // Sensors are in reverse order, 0123 are 2_2, 2_1, 1_2, and 1_1
            // respectively
            temp_sensor_voltages[ic][NUM_TEMPS - 1 - i]
                = _aux_voltages[ic][0]; // Store temperatures
        }
    }
    ...
    for (int ic = 0; ic < TOTAL_IC; ic++) {
        sprintf(temp_msg,
                "t%d,%3d,%5u,%5u,%5u,%5u,%5u,%5u,%5u,%5u,%5u,%5u,%5u,%5u,%5u,%"
                "5u,%5u,%5u,%5u,%5u,%5u,%5u",
                ic, error, temp_sensor_voltages[ic][0],
                temp_sensor_voltages[ic][1], temp_sensor_voltages[ic][2],
                temp_sensor_voltages[ic][3], temp_sensor_voltages[ic][4],
                temp_sensor_voltages[ic][5], temp_sensor_voltages[ic][6],
                temp_sensor_voltages[ic][7], temp_sensor_voltages[ic][8],
                temp_sensor_voltages[ic][9], temp_sensor_voltages[ic][10],
                temp_sensor_voltages[ic][11], temp_sensor_voltages[ic][12],
                temp_sensor_voltages[ic][13], temp_sensor_voltages[ic][14],
                temp_sensor_voltages[ic][15], temp_sensor_voltages[ic][16],
                temp_sensor_voltages[ic][17], temp_sensor_voltages[ic][18],
        ...
        for (uint8_t ic = 0; ic < TOTAL_IC; ic++) {
            for (uint8_t sensor = 0; sensor < NUM_TEMPS; sensor++) {
                ...
                if (sensor_value < SOFT_OT_THRESHOLD) {
...
#if (VERBOSE == 2)
                    sprintf(temp_msg, "SOT ic: %u cell: %u v: %u", ic, sensor,
                            sensor_value);
                    LOG_println(temp_msg, strlen(temp_msg));
#endif
                    ...
                    gFlag |= OVER_TEMP;
#if (VERBOSE == 2)
                    sprintf(temp_msg, "OT ic: %u cell: %u v: %u", ic, sensor,
                            sensor_value);
                    LOG_println(temp_msg, strlen(temp_msg));
#endif
                    ...
                    gFlag |= UNDER_TEMP;
#if (VERBOSE == 2)
                    sprintf(temp_msg, "UT ic: %u cell: %u t: %u", ic, sensor,
                            sensor_value);
                    LOG_println(temp_msg, strlen(temp_msg));
#endif
                }
...
int main(void) {
    ...
    while (1) {
        ...
        if (gFlag & TRANSMIT_STATUS) {
            ...
            gStatusMessage[6] = 0;
            for (int j = 0; j < 32; j++) {
                CAN_transmit(0, CAN_ID_BMS_CORE, CAN_LEN_BMS_CORE,
                             gStatusMessage);
            }
            ...
            if ((gFlag & OVER_VOLTAGE) || (gFlag & UNDER_VOLTAGE)
                ...
                RELAY_PORT &= ~_BV(
                    RELAY_PIN); // Commented out to prevent Shutdown of car.
                gBMSStatus = 0x00;
                LED_PORT &= ~_BV(LED3_PIN);
                EXT_LED_PORT &= ~_BV(EXT_LED2_PIN);

                // if (gFlag & OVER_VOLTAGE) {char flagmsg[]= "Over Voltage";
                // LOG_println(flagmsg, strlen(flagmsg));} if (gFlag &
                // UNDER_VOLTAGE) {char flagmsg[] = "Under Voltage";
                // LOG_println(flagmsg, strlen(flagmsg));} if (gFlag &
                // OVER_TEMP) {char flagmsg[] = "Over Temp";
                // LOG_println(flagmsg, strlen(flagmsg));} if (gFlag &
                // UNDER_TEMP) {char flagmsg[] = "Under Temp";
                // LOG_println(flagmsg, strlen(flagmsg));} if
                // (missed_cycle_count >= MAX_MISSED_CYCLES) {char flagmsg[] =
                // "Missed more than `MAX_MISSED_CYCLES`"; LOG_println(flagmsg,
                // strlen(flagmsg));}
            }
        }
    }
