...
void ltc6811_adcv(uint8_t MD, // ADC Mode
    ...
    _delay_us(4); // For up to 15 boards in series. See pg 69 in the ltc6804
                  // datasheet.
    SPI_end();
    _delay_us(4);
}
...
ltc6811_rdcv(uint8_t total_ic, // the number of ICs in the system
    ...
    const uint8_t CELL_IN_REG
        = 3; // Number of cells represented by CV register group
    ...
    uint8_t rx_data[NUM_RX_BYT * TOTAL_IC]; // Single received data stream from
                                            // all ICs on isoSPI bus
    ...
    for (uint8_t cell_reg = 1; cell_reg < (NUM_CV_REG + 1); cell_reg++) {
        ...
        for (uint8_t current_ic = 0; current_ic < total_ic; current_ic++) {
            ...
            for (uint8_t current_cell = 0; current_cell < CELL_IN_REG;
                ...
                parsed_cell
                    = rx_data[data_counter] + (rx_data[data_counter + 1] << 8);
                // create the parsed cell voltage code

                cell_codes[current_ic]
                          [current_cell + ((cell_reg - 1) * CELL_IN_REG)]
                    = parsed_cell;
            ...
            // The received PEC for the current_ic is transmitted as the 7th and
            // 8th
            received_pec = (rx_data[data_counter] << 8)
                           + rx_data[data_counter + 1]; // TODO << 8?

            // after the 6 cell voltage data bytes
            data_pec
                = pec15_calc(BYT_IN_REG, &rx_data[current_ic * NUM_RX_BYT]);
            if (received_pec != data_pec) {
                // The pec_error variable is simply set negative if any PEC
                // errors are detected in the serial data
                pec_error = -1;
            }
...
void _ltc6811_rdcv_reg(
    ...
    const uint8_t REG_LEN
        = 8; // number of bytes in each CV register group + 2 bytes for the PEC
    uint8_t cmd[4];
    uint16_t cmd_pec;
...
int8_t ltc6811_rdaux(
    ...
    // If 0, read all of em.
    if (reg == 0) {
        for (uint8_t gpio_reg = 1; gpio_reg < NUM_GPIO_REG + 1;
             gpio_reg++) { // executes once for each of the ltc6811 aux voltage
                           // registers
            data_counter = 0;
            _ltc6811_rdaux_reg(gpio_reg, total_ic,
                               data); // Reads the raw auxiliary register data
                                      // into the data[] array

            for (uint8_t current_ic = 0; current_ic < total_ic;
                 current_ic++) { // executes for every ltc6811 in the daisy
                                 // chain
                // current_ic is used as the IC counter

                for (uint8_t current_gpio = 0; current_gpio < GPIO_IN_REG;
                     current_gpio++) { // This loop parses the read back data
                                       // into GPIO voltages, it
                    // loops once for each of the 3 gpio voltage codes in the
                    // register

                    parsed_aux = data[data_counter]
                                 + (data[data_counter + 1]
                                    << 8); // Each gpio codes is received as two
                                           // bytes and is combined to
                    // create the parsed gpio voltage code

                    aux_voltages[current_ic]
                                [current_gpio + ((gpio_reg - 1) * GPIO_IN_REG)]
                        = parsed_aux;
                    data_counter
                        = data_counter
                          + 2; // Because gpio voltage codes are two bytes the
                               // data counter must increment by two for each
                               // parsed gpio voltage code
                }

                // The received PEC for the current_ic is transmitted as the 7th
                // and 8th after the 6 gpio voltage data bytes
                received_pec
                    = (data[data_counter] << 8) + data[data_counter + 1];

                data_pec
                    = pec15_calc(BYT_IN_REG, &data[current_ic * NUM_RX_BYT]);
                if (received_pec != data_pec) {
                    pec_error = -1; // The pec_error variable is simply set
                                    // negative if any PEC errors are detected
                                    // in the received serial data
                }

                data_counter
                    = data_counter + 2; // Because the transmitted PEC code is 2
        ...
        for (int current_ic = 0; current_ic < total_ic;
             current_ic++) // executes for every ltc6811 in the daisy chain
        {
                 ...
                 current_gpio++) // This loop parses the read back data. Loops
            {
                ...
                // create the parsed gpio voltage code
                aux_voltages[current_ic]
                            [current_gpio + ((reg - 1) * GPIO_IN_REG)]
                    = parsed_aux;
                data_counter
                    = data_counter + 2; // Because gpio voltage codes are two
...
void wrcomm(uint8_t total_ic, // The number of ICs being written to
                       ...
                       // starts with
    {
                             ...
                             // register
        {
            ...
            cmd[cmd_index]
                = comm[current_ic - 1][current_byte]; // adding the config data
                                                      // to the array to be sent
            cmd_index = cmd_index + 1;
        }
        comm_pec = (uint16_t)pec15_calc(
            BYTES_IN_REG,
            &comm[current_ic - 1][0]); // calculating the PEC for each ICs
...
void stcomm(void) {
    ...
    SPI_transfer_array(
        cmd, null,
        4); // Read the configuration data of all ICs on the daisy chain into
...
int8_t rdcomm(uint8_t total_ic, // Number of ICs in the system
              uint8_t r_comm[][8] // A two dimensional array that the function
    ...
    SPI_write_then_read(
        cmd, 4, rx_data,
        (BYTES_IN_REG * total_ic)); // Read the configuration data of all ICs on
                                    // the daisy chain into
    ...
    for (uint8_t current_ic = 0; current_ic < total_ic;
         current_ic++) // executes for each ltc6811 in the daisy chain and packs
                       // the data
    {
        ...
        for (uint8_t current_byte = 0; current_byte < BYTES_IN_REG;
             current_byte++) {
            r_comm[current_ic][current_byte]
...
void write_i2c(uint8_t total_ic, uint8_t address, uint8_t command,
    ...
    // Since the device only has 6 COMM registers and needs extra space for
    // other I2C actions, we can only send 3 bytes at a time over I2C. For more,
    // we need to loop through the I2C send process a couple of times, hence
    // loop_count.
    uint8_t loop_count;
    uint8_t remainder = 0;
    ...
    for (uint8_t i = 0; i < loop_count; i++) {
        if ((i == (loop_count - 1))
            && (remainder != 0)) // need to pad becuase we don't have multiple
                                 // of 3 data bytes
        {
            for (uint8_t k = 0; k < remainder; k++) {
                comm[0][transmitted_bytes]
                    = BLANK + (data[data_counter] >> 4); //
                if (k != (remainder - 1)) {
                    comm[0][transmitted_bytes + 1]
                        = (data[data_counter] << 4) | NACK;
                } else {
                    comm[0][transmitted_bytes + 1]
                        = (data[data_counter] << 4) | NACK_STOP;
                }
...
void mux_set_channel(uint8_t total_ic, uint8_t i2c_address, uint8_t channel) {
    ...
    write_i2c(total_ic, i2c_address, command, 0,
              0); //(total_ic, address, command, data, data_length)
}
