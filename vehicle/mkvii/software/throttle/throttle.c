...
#include "vehicle/mkvii/software/throttle/can_api.h"
...
#define SET_TORQUE_REQUEST(torque) \
    (m192_command_message.vcu_torque_command = (torque))
...
int main(void) {
    ...
    m192_command_message.vcu_direction_command = MOTOR_CLOCKWISE;
    ...
    while (true) {
        ...
        if (run_1ms) {
            ...
            if (can_poll_receive_dashboard() == 0) {
                // check RTD status
                can_receive_dashboard();
                m192_command_message.vcu_inverter_enable = dashboard.ready_to_drive;
            }
